...
$(document).ready(function () {
	...
	function initGMap ( mapSelector ) {
		...
		if ( !gMapAPILoaded ) {
			$.getScript( "//maps.google.com/maps/api/js?key=AIzaSyAwH60q5rWrS8bXwpkZwZwhw9Bw0pqKTZM&sensor=false&libraries=geometry,places&v=3.7", init );
		} else {
			init();
		}
	}
	...
	$(".slider-range").slider({
		...
		slide: function (event, ui) {
			...
			$(".amount2").val(ui.values[1] + "đ");
		}
	});
	$(".amount").val($(".slider-range").slider("values", 0) + "đ");
	$(".amount2").val($(".slider-range").slider("values", 1) + "đ");
